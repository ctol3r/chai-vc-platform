...
import { record } from '../services/audit_scrapbook';
...
async function fetchNpiData(npi: string): Promise<any> {
...
export function pilotRoutes() {
  const r = Router();

  // Health check
  r.get('/health', (_req: Request, res: Response) => res.json({ ok: true, service: 'backend' }));

  // Issue credential
  r.post('/issuer/credential', async (req: Request, res: Response) => {
    try {
      ...
      await store.setIssued(credentialId, { jwt, subjectId: subject.id || 'did:example' });
      const auditRef = await record('issue', { credentialId });
  ...
  // Revoke credential
  r.post('/issuer/revoke', async (req: Request, res: Response) => {
    try {
  ...
  r.post('/verifier/presentation', async (req: Request, res: Response) => {
    try {
      // Validate JSON body
      if (!req.body || typeof req.body !== 'object') {
        return res.status(400).json({
          ...
          auditRef: await record('verify_error', { error: 'invalid_json_body' }),
        });
      }
      ...
      if (!jwt) {
        return res.status(400).json({
          valid: false,
          reason: 'missing_jwt',
          auditRef: await record('verify_error', { error: 'missing_jwt' }),
        });
      }
      ...
      if (!credentialId) {
        return res.json({
          ...
          auditRef: await record('verify_error', { error: 'unknown_credential' }),
        });
      }
      ...
      res.status(500).json({
        ...
        auditRef: await record('verify_error', { error: 'internal_error' }),
      });
    }
  });

  // NPI lookup with caching
  r.post('/lookup/npi/:npi', async (req: Request, res: Response) => {
    try {
      ...
      if (cached && Date.now() - cached.timestamp < NPI_CACHE_TTL) {
        return res.json({
          ...
          data: cached.data,
          cached: true,
          auditRef: await record('npi_lookup_cached', { npi }),
        });
      }
      ...
      res.json({
        ...
        data,
        cached: false,
        auditRef: await record('npi_lookup', { npi }),
      });
    } catch (error: any) {
      res.status(500).json({
        ok: false,
        error: error.message || 'npi_lookup_failed',
        auditRef: await record('npi_lookup_error', { npi: req.params.npi, error: error.message }),
      });
    }
  });

  // FHIR Practitioner export
  r.get('/fhir/Practitioner/:id', async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
