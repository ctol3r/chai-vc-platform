...
export class VerifierStakingService {
    ...
    async stake(amount: BigNumberish) {
        return await (this.contract as unknown as { stake: (options: any) => Promise<any> }).stake({ value: amount });
    }
    ...
    async withdraw(amount: BigNumberish) {
        return await (this.contract as unknown as { withdraw: (value: any) => Promise<any> }).withdraw(amount);
    }
    ...
    async slash(verifier: string, amount: BigNumberish) {
        return await (this.contract as unknown as { slash: (address: string, value: any) => Promise<any> }).slash(verifier, amount);
    }
}
